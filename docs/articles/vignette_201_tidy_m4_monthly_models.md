...
In this example, we will use the `m4_monthly_subset`. The dataset is a
monthly `tsibble`, which is filtered to include only the time series
`"M21655"` and `"M2717"`. The resulting object `train_frame` contains
the training data and is visualized below.
...
![Plot actual
values](vignette_201_tidy_m4_monthly_models_files/figure-html/data-1.png)
...
data](vignette_201_tidy_m4_monthly_models_files/figure-html/fable_frame-1.png)
