...
By default, `train_esn()` automatically determines the number of
differences, constructs the reservoir, evaluates candidate ridge readout
models, and selects the final readout using an information criterion.
For explicit grid-based tuning of `alpha`, `rho`, and `tau`, use
`tune_esn()`.
