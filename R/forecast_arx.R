...
#' @title Forecast a trained Autoregressive model
#' 
#' @description Forecast a trained Autoregressive model.
#' 
#' @param object An object of class \code{ARX}. The result of a call to \code{train_arx()} or \code{auto_arx()}.
...
#' @title Forecast a trained Autoregressive model (internal function)
#' 
#' @description Calculate point forecasts of a trained Autoregressive model
#'   (internal function).
#' 
...
predict_arx <- function(wout,
  ...
  for (t in 2:(n_ahead + 1)) {
    ...
    Xf <- cbind(1, inputs[t, , drop = FALSE])
